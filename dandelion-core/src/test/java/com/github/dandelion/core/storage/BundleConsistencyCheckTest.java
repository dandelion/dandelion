...
public class BundleConsistencyCheckTest {
	...
	public void setup() {
		...
		String path = new File("src/test/resources/bundle-loading/json/consistency-check/dandelion".replace("/", File.separator))
				.getAbsolutePath();
	...
	public void should_throw_an_exception_because_of_bundle_consistency() {
		...
		exception
				.expectMessage("[bundle-with-error] The bundle contain asset with no location whereas it is required.");
		...
		exception
				.expectMessage("[missing-extension] The extension is required in all locations.");
